 ...
 */
class APIV1Spec extends AbstractSpringV1ApiWebIntegrationSpec implements HttpAssertionsTrait, CountryAssertionsTrait {
    ...
    def "'/name/russian federation' with full text option returns the appropriate country data"() {
        when:
            def response = apiClient.get(path: 'name/russian federation', query: [fullText: 'true'])
    ...
    def "'/name/russian fed' without full text option returns HTTP 404"() {
        when:
            def response = apiClient.get(path: 'name/russian fed', query: [fullText: 'true'])
